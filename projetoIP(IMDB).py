...
def reiniciar():
    usuario_sabe_filme = str(input('Você sabe qual filme deseja assistir [s/n]? ')).upper()[0].strip()
...
p = str(input('Você sabe qual filme deseja assitir [s/n]? ')).upper()[0].strip()

while p not in 'SN':
    p = str(input('Você sabe qual filme deseja assistir [s/n]? ')).upper()[0].strip()
...
if p == 'S':
    ...
    while opcao not in range(1, 11):
        opcao = int(input('Escolha uma opção: '))
    filme_entrada = filme_entrada[opcao - 1]
